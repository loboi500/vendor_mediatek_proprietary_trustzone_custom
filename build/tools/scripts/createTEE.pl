...
{
  if (index($base_project_cfg, "vext_") != -1) {
    my $base_proj_tmp = getOptionValue($base_project_cfg, "VEXT_BASE_PROJECT");
    print "INFO: VEXT_BASE_PROJECT is $base_proj_tmp\n";
    if (($base_proj_tmp eq "") || ($base_proj_tmp eq "\$(MTK_TARGET_PROJECT)")) {
      die "ERROR: flavor project but VEXT_BASE_PROJECT incorrect\n";
    }
  } else {
